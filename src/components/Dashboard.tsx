...
import ColorMixerModal from './ColorMixerModal'
...
export default function Dashboard() {
	...
	const [isColorMixerModalOpen, setIsColorMixerModalOpen] = useState(false)
	...
	useEffect(() => {
		setCurrentPage(1)
	}, [searchTerm, selectedCategory, selectedGroup, sortField, sortOrder])

	const loadData = async () => {
		try {
			setIsLoading(true)
			const [fetchedColors, fetchedCategories, fetchedGroups] =
				await Promise.all([getColors(), getCategories(), getGroups()])
	...
	const filteredColors = colors
		.filter(color => {
			...
			const matchesGroup =
				selectedGroup === 'all' || color.group === selectedGroup
	...
	return (
		<div className={`min-h-screen ${isDark ? 'bg-gray-900' : 'bg-gray-50'}`}>
					...
					<div className='flex-1 overflow-y-auto py-6'>
								...
								{categories.map(category => (
									<div key={category} className='flex items-center group'>
										<button
											...
											className={`flex-1 text-left px-4 py-2.5 rounded-xl transition-all duration-200 ${
						...
						<div className='px-6 mb-6'>
							...
							<div className='space-y-1'>
								...
								{groups.map(group => (
									<div key={group} className='flex items-center group'>
										<button
											onClick={() => {
										...
										{user?.isAdmin && (
											<button
												onClick={() => {
													if (
														confirm(
															`Вы точно хотите удалить группу "${group}"?`
														)
													) {
			...
			<ColorMixerModal
				isOpen={isColorMixerModalOpen}
				onClose={() => setIsColorMixerModalOpen(false)}
			/>
