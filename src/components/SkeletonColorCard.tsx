    ...
    <div className={`rounded-lg shadow-md overflow-hidden ${isDark ? 'bg-gray-800' : 'bg-white'}`}>
      ...
      <div className="p-3 sm:p-4 space-y-3">
        <div className="space-y-2">
          <div className={`h-6 rounded ${shimmerClass} ${isDark ? 'bg-gray-700' : 'bg-gray-200'}`} />
          <div className={`h-4 w-24 rounded ${shimmerClass} ${isDark ? 'bg-gray-700' : 'bg-gray-200'}`} />
        </div>

        <div className={`p-2 sm:p-3 rounded-lg space-y-2 ${isDark ? 'bg-blue-900/30' : 'bg-blue-50'}`}>
          <div className="flex items-center gap-2 mb-2">
            <div className={`w-4 h-4 rounded ${shimmerClass} ${isDark ? 'bg-gray-700' : 'bg-gray-200'}`} />
            <div className={`h-4 w-16 rounded ${shimmerClass} ${isDark ? 'bg-gray-700' : 'bg-gray-200'}`} />
          </div>
          <div className={`h-4 rounded ${shimmerClass} ${isDark ? 'bg-gray-700' : 'bg-gray-200'}`} />
          <div className={`h-4 rounded ${shimmerClass} ${isDark ? 'bg-gray-700' : 'bg-gray-200'}`} />
        </div>
