...
const themeClasses = {
  input: {
    light: 'border-gray-200 bg-white text-gray-900 focus:border-blue-500 focus:ring-blue-200',
    dark: 'border-gray-700 bg-gray-800 text-gray-100 focus:border-blue-400 focus:ring-blue-500/20',
  },
  button: {
    primary: {
      light: 'bg-blue-600 text-white hover:bg-blue-700 active:bg-blue-800 shadow-sm hover:shadow',
...
export default function EditColorModal({
  ...
  const [alternativeName, setAlternativeName] = useState(color.alternativeName || '');
  ...
  const [isVerified, setIsVerified] = useState(color.isVerified || false);

  useEffect(() => {
    setName(color.name);
    setAlternativeName(color.alternativeName || '');
    setHex(color.hex);
    ...
    setIsVerified(color.isVerified || false);
