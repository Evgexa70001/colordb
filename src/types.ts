...
// Тип для причин создания новых цветов
export interface ColorCreationReason {
	id: string
	colorId: string
	colorName: string
	createdAt: Date
	...
	technicalRequirements?: {
		...
		actualLightingConditions?: string[] // условия освещения при создании
		actualSubstrateCompatibility?: string[] // материалы, на которых создан цвет
		actualDurabilityRequirements?: string[] // требования к долговечности созданного цвета
		achievedColorValues?: {
			lab: { l: number; a: number; b: number }
		}
		productionNotes?: string // заметки о процессе создания
	}
	status: 'proposed' | 'approved' | 'in_development' | 'completed' | 'rejected'
	approvedBy?: string
	approvedAt?: Date
	completionDate?: Date
	attachments?: string[] // URLs к документам
	notes?: string
}

// Типы для входного контроля
export interface ReferencePaint {
	id: string
	name: string
	supplier: string
	labValues: {
